...
jQuery(document).ready(function () {
    jQuery(document).ready(function () {
        //hide all inputs radio
        jQuery('.cmb-type-grid-radio ul li input').hide();

        //add border for checked image when click
        jQuery('.cmb-type-grid-radio ul li img').on('click', function () {
            ...
            var dep_id = jQuery(this).parents('li').find('input').attr('id');

            if (jQuery(this).parents('div.inside').find('div[data_id*="' + dep_id + '"]').length > 0) {
                jQuery(this).parents('div.inside').find('div[data-role="' + dep_id.slice(0, -1) + '"]').hide();
                jQuery(this).parents('div.inside').find('div[data_id*="' + dep_id + '"]').each(function () {
                    jQuery(this).show();
                });

                jQuery(this).parents('li').find('input').prop('checked', true);
            } else {
                jQuery(this).parents('li').find('input').prop('checked', true);
                jQuery(this).parents('div.inside').find('div[data-role*="' + dep_id.slice(0, -1) + '"]').hide();
            }
        ...
        //add border for checked image when page onload
        jQuery('.cmb-type-grid-radio ul li').find('input').each(function () {
            if (jQuery(this).is(':checked')) {
                ...
                jQuery(this).parents('li').find('img').css(
                    {
                        'border': '5px solid grey'
                    }
                );
        ...
        // this is used on change
        jQuery('#_cmb2_minimal_post_type').on('change', function () {
            var dinamic_id,
                selected_post_type = jQuery('option:selected', this).val();

            dinamic_id = 'div[data_id*="_cmb2_minimal_post_type' + selected_post_type + '"]';

            jQuery('.generic-params').val(selected_post_type);

            // first hide all the inputs
            jQuery(this).parents('div.inside').find('div[data_id*="_cmb2_minimal_post_type"]').hide();


            // use set time out if you notice that the elemtns do not show up on time
            //setTimeout(function(){
            // show only the necessary option
            jQuery(dinamic_id).show();
            //},3);
        });


        // this is used on page load
        var current_selected_post_type_id, onload_selected_post_type;
        if (jQuery('#_cmb2_minimal_post_type option:selected').length) {
            onload_selected_post_type = jQuery('#_cmb2_minimal_post_type option:selected').val()
            jQuery('.generic-params').val(onload_selected_post_type);

            //first hide all the post_type meta options
            jQuery('#_cmb2_minimal_post_type').parents('div.inside').find('div[data_id*="_cmb2_minimal_post_type"]').hide();

            current_selected_post_type_id = 'div[data_id*="_cmb2_minimal_post_type' + onload_selected_post_type + '"]';
            jQuery(current_selected_post_type_id).show();
        }

        jQuery('#cmb2_minimal-metabox-latest_galleries .generic-params ').val('gallery');
    })
})