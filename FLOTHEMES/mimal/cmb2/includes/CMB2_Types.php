...
class CMB2_Types {
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<button data-selector="<?php echo $table_id; ?>" class="cmb-add-row-button button"><?php echo esc_html( $this->_text( 'add_row_text', __( 'Add Row', 'flotheme' ) ) ); ?></button>
		</p>
	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo $class; ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button class="button cmb-remove-row-button<?php echo $disabled; ?>"><?php echo esc_html( $this->_text( 'remove_row_text', __( 'Remove', 'flotheme' ) ) ); ?></button>
			</div>
		</div>
...
//        var_dump($this->field->value,$a['options']);die;
//        $this->field->options(),$a['options'];
        $opt = '';
        foreach($this->field->options() as $k=>$option){
            if($this->field->value == $k){
                $opt .= '<option value="'.$k.'" id='.$this->_name().$k.' selected="selected" >'.$option.'</option>';
            }else{
                $opt .= '<option value="'.$k.'" id='.$this->_name().$k.' >'.$option.'</option>';
            }
        }
		$attrs = $this->concat_attrs( $a, array( 'desc', 'options' ) );
		return sprintf( '<select%s>%s</select>%s', $attrs, $opt, $a['desc'] );
	}
    ...
    public function grid_radio( $args = array(), $type = 'radio' ) {
//        var_dump($type);die;
        extract( $this->parse_args( $args,  $type, array(
            'class'   => 'cmb2-radio-list cmb2-list',
            'options' => $this->concat_items( array( 'label' => 'test', 'method' => 'grid_input' ) ),
            'desc'    => $this->_desc( true ),
        ) ) );
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= sprintf( '<li><label>%s</label></li>', esc_html( $this->_text( 'no_terms_text', __( 'No terms', 'flotheme' ) ) ) );
		} else {
			$option_none  = $this->field->args( 'show_option_none' );
			if ( ! empty( $option_none ) ) {
     ...
     * @return string       Gnerated list item html
     */
    public function grid_input( $args = array(), $i ) {
//        var_dump($i, 'dd');
        $args = $this->parse_args( $args, 'grid_input', array(
            'type'  => 'radio',
            'class' => 'cmb2-option',
            'name'  => $this->_name(),
            'id'    => $this->_id( $i ),
        ...
        $path   = isset( $this->field->args['path'] ) ? $this->field->args['path'] : '';

        $result = '';
        $action1 = array();
        $return = '';
        $c = 1;
        foreach( $this->field->args['value']  as $index => $val ){
            $value[$index]['image'] = $val;
        }
        foreach( $this->field->args['options']  as $index => $val ){
            $value[$index]['title'] = $val;
        }

        $j = 1;
        foreach($value as $k=>$v){
            $result = '';
            $result = '<img title="' . $v['title'] . '" class="pattern-texture ' . $k. '" alt="' . $v['title'] . '" src="' . get_template_directory_uri() . '/flo-options/ReduxCore/assets/img/' . $path . $v['image'] . '" />';
            $return = sprintf( '<li><input%s/> %s <label for="%s">%s</label></li>'."\n", $this->concat_attrs( $args, 'label' ),$result, $args['id'], $args['label'] );
            if($i == $j){
                return $return;
            }
            $j++;
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html( $this->_text( 'add_upload_files_text', __( 'Add or Upload Files', 'flotheme' ) ) ),
			'name'  => '', 'id'  => '',
		) );
	...
	public function file() {
		...
		printf( '<input class="cmb2-upload-button button" type="button" value="%s" />', esc_attr( $this->_text( 'add_upload_file_text', __( 'Add or Upload File', 'flotheme' ) ) ) );
	...
	public function img_status_output( $args ) {
		printf( '<%1$s class="img-status">%2$s<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>',
			...
			esc_html( $this->_text( 'remove_image_text', __( 'Remove Image', 'flotheme' ) ) ),
	...
	public function file_status_output( $args ) {
		printf( '<%1$s class="file-status"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="cmb2-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>',
			$args['tag'],
			esc_html( $this->_text( 'file_text', __( 'File:', 'flotheme' ) ) ),
			$this->get_file_name_from_path( $args['value'] ),
			$args['value'],
			esc_html( $this->_text( 'file-download-text', __( 'Download', 'flotheme' ) ) ),
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html( $this->_text( 'remove_text', __( 'Remove', 'flotheme' ) ) ),
			isset( $args['id_input'] ) ? $args['id_input'] : ''
		);
	}
