...
//	}
    ...
    public function render_field() {
        ...
        if($this->args( 'dependence_parent_id' ) && $this->args( 'dependence_option_index' )){
            if(is_array($this->args( 'dependence_option_index' ))){
                $dependence ='';$dep = '';
                foreach ($this->args( 'dependence_option_index' ) as $k=> $v) {
                    $dep .= $this->args('dependence_parent_id') . '' . $v.',';
                }
                $dependence =  'data_id="' . $dep . '"';

            }else {
                $dependence = 'data_id="' . $this->args('dependence_parent_id') . '' . $this->args('dependence_option_index') . '"';
            }
        }else{
            $dependence = '';
        }
//        echo $dependence;
        ...
        $this->peform_param_callback( 'before_row' );
        if($dependence != ''){
            printf( "<div class=\"cmb-row %s\" %s style='display: none' data-role='%s'>\n", $this->row_classes(), $dependence,$this->args('dependence_parent_id') );
        }else{
            printf( "<div class=\"cmb-row %s\">\n", $this->row_classes());
        }
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => __( 'Add Group', 'flotheme' ),
			'remove_button' => __( 'Remove Group', 'flotheme' ),
		) ) : $args['options'];
		...
		if ( in_array( $args['type'], $option_types, true ) ) {
			...
			$args['show_option_none'] = true === $args['show_option_none'] ? __( 'None', 'flotheme' ) : $args['show_option_none'];

			if ( ! $args['show_option_none'] ) {
				$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
				$args['show_option_none'] = $off_by_default ? false : __( 'None', 'flotheme' );
			}
