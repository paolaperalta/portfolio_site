...
class widget_flickr extends WP_Widget {
    function __construct() {
        ...
        /* Constructor */
        $widget_ops = array('classname' => 'flo-widget widget_flickr_photos', 'description' => __( 'Flickr Photos' , 'flotheme' ) );
        parent::__construct('widget_cosmo_flickrwidget', _TN_ . ' : ' . __( 'Flickr Photos' , 'flotheme' ), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        $title = empty($instance['title']) ? __('Photo Gallery','flotheme') : apply_filters('widget_title', $instance['title']);
...
?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of photos','flotheme') ?>:
                <input class="widefat" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo esc_attr($number); ?>" />
            </label>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('showing'); ?>"><?php _e('Showing Method','flotheme') ?>:
                <select size="1" name="<?php echo $this->get_field_name('showing'); ?>">
                    <option value="random"<?php if(esc_attr($showing) =='random'){echo 'selected';}?>><?php _e('Random Photo','flotheme')?></option>
                    <option value="latest"<?php if(esc_attr($showing) =='latest'){echo 'selected';}?>><?php _e('Latest Photo','flotheme') ?></option>
                </select>
            </label>
        </p>
<?php
