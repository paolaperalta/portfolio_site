...
(function( $ ) {
    ...
    $.redux.initEvents = function() {
        ...
        $( '#toplevel_page_' + redux.args.slug + ' .wp-submenu a, #wp-admin-bar-' + redux.args.slug + ' a.ab-item' ).click(
            function( e ) {
				//check if this is a theme update page By Sliva
	            if($(this).attr('href') == 'admin.php?page=flotheme_updater'){
		            return true;
	            }else if ( ( $( '#toplevel_page_' + redux.args.slug ).hasClass( 'wp-menu-open' ) || $( this ).hasClass( 'ab-item' ) ) && !$( this ).parents( 'ul.ab-submenu:first' ).hasClass( 'ab-sub-secondary' ) ) {
                    e.preventDefault();
