    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                add_action('wp_ajax_importDummyData' , array('ReduxFramework', 'importDummyData')); // for one click importer
                
                add_action('wp_ajax_importDemoSettings' , array('ReduxFramework', 'importDemoSettings')); // for one click importer
                ...
                if ( empty( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'flotheme' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'flotheme' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'flotheme' );
                }

                if ( empty( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'flotheme' );
                }
...
//                    }
                ...
                global $flo_options;
                if(isset($flo_options['flo_minimal-change_gallery_shortcode']) && $flo_options['flo_minimal-change_gallery_shortcode'] == 'yes'){
                    add_filter( 'post_gallery', 'flo_post_gallery', 10, 2 ); //overwrite the default WP gallery shortcode
                }
            } // __construct()
            ...
            private function set_default_args() {
                $this->args = array(
                    ...
                    'footer_credit'             => 'Flothemes Options Panel',
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'flotheme' ),
...
//                }
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'flotheme' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'flotheme' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'flotheme' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'flotheme' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'flotheme' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'flotheme' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty( $default_output ) ) {
                    $default_output = __( 'Default', 'flotheme' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function get_header_html( $field, $section_name = '' ) {
                global $current_user;
                ...
                if ( isset( $field['title'] ) && isset( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
                    ...
                    if ( isset($field['filter']) ) {
                        $filter = true;
                        $data_title .= ' '. $field['filter'];
                    }

                    if ($filter) {
                        $data_title = 'data-filter="' . strtolower( $data_title ) . '" ';

                        $data_title .= 'data-pagename="' . $section_name . '" ';
                    }
                ...
                if ( ! empty( $th ) ) {
                    $th = '<div class="redux_field_th" ' . $data_title . '>' . $th . '</div>';
                }
...
//                            }
                            ...
                            $th = $this->get_header_html( $field, $section_title );
...
//                }
            ...
            public function _options_page_html() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'flotheme' ) . '</div></noscript>';
                ...
                if ( ! empty( $this->args['display_name'] ) ) {
                    ...
                    if ( isset( $this->args['dev_mode'] ) && $this->args['dev_mode'] ) {
                        echo '<span class="redux-dev-mode-notice">' . __( 'Developer Mode Enabled', 'flotheme' ) . '</span>';
                    }
                ...
                echo '<a href="javascript:void(0);" class="expand_options' . $expanded . '"' . $hide_expand . '>' . __( 'Expand', 'flotheme' ) . '</a>';
                echo '<div class="redux-action_bar">';
                submit_button( __( 'Save Changes', 'flotheme' ), 'primary', 'redux_save', false );

                if ( false === $this->args['hide_reset'] ) {
                    echo '&nbsp;';
                    submit_button( __( 'Reset Section', 'flotheme' ), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
                    echo '&nbsp;';
                    submit_button( __( 'Reset All', 'flotheme' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
                }

                echo '</div>';

                // Search option
                echo '<input type="text" value="" name="redux_filter" id="redux-option-filter" placeholder="'. __('Search an option in here', 'flotheme') .'">';
                echo '<div class="clear-filter">Esc &#10005;</div>';
                //
                
                echo '<div class="redux-ajax-loading" alt="' . __( 'Working...', 'flotheme' ) . '">&nbsp;</div>';
                echo '<div class="clear"></div>';
                echo '</div>';
                ...
                if ( isset( $this->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->args['opt_name']}", __( 'Settings Imported!', 'flotheme' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->args['opt_name']}", __( 'All Defaults Restored!', 'flotheme' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->args['opt_name']}", __( 'Section Defaults Restored!', 'flotheme' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'flotheme' ) ) . '</strong></div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->args['opt_name']}", __( 'Settings have changed, you should save them!', 'flotheme' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . __( 'error(s) were found!', 'flotheme' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . __( 'warning(s) were found!', 'flotheme' ) . '</strong></div>';
                ...
                if ( $this->args['system_info'] === true ) {
                    ...
                    echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'flotheme' ) . '</span></a>';
                    echo '</li>';
                }
                ...
                if ( $this->args['system_info'] === true ) {
                    ...
                    echo '<h3>' . __( 'System Info', 'flotheme' ) . '</h3>';
...
//                    $class_file = apply_filters( 'redux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                    ...
                    */
                        ...
                        if ( isset( $field['help_img'] ) && ! '' == $field['help_img'] ) {
                            $hint_id = mt_rand(0,99999);
                            $help_img = $field['help_img'];
                            if(is_array($help_img)){
                                $help_img_content =
                                    '<div style="float:right;margin-top: 20px;margin-left: 20px;" title="'.__
                                    ('See how it looks on the site','flotheme').'" >';
                                foreach($help_img as $img){
                                    $help_img_path = 'http://flothemes.com/wp-content/uploads/dashboard-images/generosity';
//                                $help_img_path = get_template_directory_uri().'/flo-options/ReduxCore/assets/img/hints';
                                    $help_img_content .= '<a href="'.$help_img_path.'/'.$img.'"
                                data-fancybox-group="prettyPhoto_'.$hint_id.'" title="'.__('See how it looks on the
                                site','flotheme').'" class="img-lightbox-hint" target="_blank">';
                                }
                                $help_img_content .= '<span class="dashicons dashicons-editor-help"></span>
                                            </a></div>';
                            }else{
...
//                                $help_img_path = get_template_directory_uri().'/flo-options/ReduxCore/assets/img/hints';
                                    $help_img_content =
                                        '<div style="float:right;margin-top: 20px;margin-left: 20px;" title="'.__('See how it looks on the site','flotheme').'" ><a href="'.$help_img_path.'/'.$help_img.'" data-fancybox-group="prettyPhoto_'.$hint_id.'" title="'.__('See how it looks on the site','flotheme').'" class="img-lightbox-hint" target="_blank">
                        ...
                        if ( isset( $field['doc_iframe'] ) && ! '' == $field['doc_iframe'] ) {
                            $hint_id = mt_rand(0,99999);
                            $doc_iframe = $field['doc_iframe'];
                            if(is_array($doc_iframe)){
                                $doc_iframe_content =
                                    '<div style="float:right;margin-top: 20px;margin-left: 20px;" title="'.__
                                    ('See how it looks on the site','flotheme').'" >';
                                foreach($doc_iframe as $img){
                                    $doc_iframe_content .= '<a href="'.$img.'"
                                ...
                                if( isset($doc_iframe) && strlen($doc_iframe) ){
                                    $doc_iframe_content =
                                        '<div style="float:right;margin-top: 20px;margin-left: 20px;" title="'.__('See how it looks on the site','flotheme').'" >
                                        <a href="'.$doc_iframe.'" data-fancybox-type="iframe" data-fancybox-group="prettyPhoto_'.$hint_id.'" class="video-lightbox-hint" title="My YouTube Video">
                        ...
                        if ( isset( $field['video_iframe'] ) && ! '' == $field['video_iframe'] ) {
                            $hint_id = mt_rand(0,99999);
                            $video_iframe = $field['video_iframe'];
                            if(is_array($video_iframe)){
                                ...
                                if( isset($video_iframe) && strlen($video_iframe) ){
                                    $video_iframe_content =
                                        '<div style="float:right;margin-top: 20px;margin-left: 20px;" title="'.__('See how it looks on the site','flotheme').'" >
                                        <a href="'.$video_iframe.'" data-fancybox-type="iframe" data-fancybox-group="prettyPhoto_'.$hint_id.'" class="video-lightbox-hint" title="My YouTube Video">
                        ...
                        echo  $video_iframe_content . $doc_iframe_content . $help_img_content;
                        //if ( ! isset( $field['fields'] ) || empty( $field['fields'] ) ) {
...
//                        }
            ...
            static function importDummyData()
            {
                ...
                require_once get_template_directory() . '/flo-options/ReduxCore/demo_content/flo-importer/wordpress-importer.php';
                global $flo_options ; // This is your opt_name.
				if(isset($_POST['folder']) && $_POST['folder'] != ''){
					$import_dir = $_POST['folder'];
				}else{
					if (isset($flo_options['flo_minimal-import_one_click']) && $flo_options['flo_minimal-import_one_click'] != "") {
						$import_dir = $flo_options['flo_minimal-import_one_click'];
					} else {
						$import_dir = 'export';
					}
				}
                $tmp_dir = get_template_directory() . '/flo-options/ReduxCore/demo_content/' . $import_dir . '/';
                if (class_exists('Flo_Import_Dummy_Data')) {
                    ...
                    if ($result && stripos($buffer, '<p>All done.') !== false) {
                        ...
                        die(json_encode(array('message' => 'Something went wrong. Please contact administrator', 'code' => -1)));
                    }
                } else {
                    die(json_encode(array('message' => 'Missing importer class. Please contact administrator.', 'code' => -1)));
                }
            ...
            /**
            *
            * Import the dumy Settings
            *
            **/
            static function importDemoSettings(){

                if(isset($_POST['folder']) && $_POST['folder'] != ''){
                    $import_dir = $_POST['folder'];
                }else{
                    if (isset($flo_options['opt-quick_import_options']) && $flo_options['opt-quick_import_options'] != "") {
                        $import_dir = $flo_options['opt-quick_import_options'];
                    } else {
                        $import_dir = 'export';
                    }
                }
                ...
                $fw_import_options = $tmp_dir.'fw_import.options'; // this it the absolute path to the settings file

                $imported_options = @json_decode( file_get_contents($fw_import_options), true ); // read the settings file and json_decode it


                update_option('flo_options',$imported_options); // update the options
           ...
           static function get_aqua_size($size_name, $width_or_height = 'width', $option = 'imagesizes')
            {
                global $flo_options,$reduxConfig;
                $name = $size_name . '-' . $width_or_height;
	            if(isset($flo_options) && isset($flo_options[$name] )&& $flo_options[$name] !=''):
                    $result = $flo_options[$name];
	            else:
		            $result = '';
		        endif;
                ...
                if (is_numeric(trim($result))) {
                    ...
                    /*if the option is empty or not numeric for some reason, we return the default*/
                    return 9999;
                }
            }
 ...
 */
include_once('attached_images_manager.php');

// slideshow manager
include_once('slideshow_attached_images_manager.php');
include_once('flo_main.php');
