...
if ( ! class_exists( 'ReduxFramework_extension_customizer' ) ) {
    ...
    class ReduxFramework_extension_customizer {
        ...
        public function _register_customizer_controls( $wp_customize ) {
            global $to_flo_main, $to_admin_js;
            $to_flo_main = $to_admin_js = array();
            $order    = array(
                'heading' => - 500,
                'option'  => - 500,
            );
            ...
            foreach ( $this->parent->sections as $key => $section ) {
                ...
                foreach ( $section['fields'] as $skey => $option ) {
                    ...
                    if ( isset($option['type'] ) && $option['type'] != 'heading' && ! isset( $option['priority'] ) ) {
                        $option['priority'] = $order['option'];
                        $order['option'] ++;
                    }

                    if ( isset($option['id']) && ! empty( $this->options_defaults[ $option['id'] ] ) ) {
                    ...
                    if ( isset($option['type']) && $option['type'] == "select" && isset( $option['multi'] ) && $option['multi'] == true ) {
                        continue;
                    }
                    if(isset($option['id'])){
                        $option['id'] = $this->parent->args['opt_name'] . '[' . $option['id'] . ']';
                    ...
                    if ( isset($option['type']) && $option['type'] != "heading" || ! empty( $option['type'] ) ) {
...
//                                    $trueID = str_replace(array(']', 'flo_options['),'',$option['id']);
//                                    $data = get_option($this->parent->args['opt_name']);
                                if(isset($option['mode'])){
                                    $wp_customize->add_control($option['id'], array(
                                        ...
                                        'settings' => $option['id'],
                                        'priority' => $option['priority'],
                                        'input_attrs' => array('class' => 'cust_fields'),
                                        'type' => 'text',
                                    ));
                                }else{
                                    $wp_customize->add_control($option['id'], array(
                                        ...
                                        'settings' => $option['id'],
                                        'priority' => $option['priority'],
                                        'input_attrs' => array('class' => isset( $option['class']) ?  $option['class'] : ''),
                                        'type' => 'text',
                                    ));
...
//                                );
	                        ...
	                        case 'image_select':
                                $trueID = str_replace(array(']', 'flo_options['),'',$option['id']);
                                // $to_admin_js is a global array used in admin.js file
                                $to_admin_js[] = 'customize-control-flo_options-'.$trueID;
                                wp_localize_script( 'jquery', 'imageSelectElements', $to_admin_js);
		                        $wp_customize->add_control($option['id'], array(
				                        'label' => $option['title'],
				                        'section' => $section['id'],
				                        'settings' => $option['id'],
                            ...
                            case 'media':
//                                continue;
                                error_reporting(E_ERROR | E_PARSE); // hide the warnings caused by the media inputs.
                                
                                $trueID = str_replace(array(']', 'flo_options['),'',$option['id']);
                                // $to_flo_main is a global array used in flo_main.php file
                                $to_flo_main[] = $trueID;
...
//                );
            ...
            $localize = array(
                'save_pending'   => __( 'You have changes that are not saved. Would you like to save them now?', 'flotheme' ),
                'reset_confirm'  => __( 'Are you sure? Resetting will lose all custom values.', 'flotheme' ),
                'preset_confirm' => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'flotheme' ),
