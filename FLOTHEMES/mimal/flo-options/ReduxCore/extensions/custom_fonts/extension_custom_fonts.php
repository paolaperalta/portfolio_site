    ...
    class ReduxFramework_extension_custom_fonts {
                ...
                foreach ( $fonts as $section ) {
                    if ( $section['type'] == "d" && ! empty( $section['name'] ) ) {
                        if ( $section['name'] == "custom" ) {
                            $section['name'] = __( 'Custom Fonts', 'flotheme' );
                        } else if ( $section['name'] == "fontsquirrel" ) {
                            $section['name'] = __( 'Fonts Squirrel', 'flotheme' );
                        }
        ...
        public function ajax() {
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "delete" ) {
                if ( $_REQUEST['section'] == __( 'Custom Fonts', 'flotheme' ) ) {
                    $_REQUEST['section'] = "custom";
                }
                if ( $_REQUEST['section'] == __( 'Fonts Squirrel', 'flotheme' ) ) {
                ...
                try {
                    ...
                    echo json_encode( array(
                        'type' => 'error',
                        'msg'  => __( 'Unable to delete font file(s).', 'flotheme' )
                    ) );
                }
        ...
        function processWebfont( $attachment_id, $name, $mime_type, $subfolder = 'custom/' ) {
            ...
            if ( empty( $path ) ) {
                echo json_encode( array(
                    'type' => 'error',
                    'msg'  => __( 'Attachment does not exist.', 'flotheme' )
                ) );
                die();
            }
            ...
            if ( $subtype == "zip" ) {
                ...
                echo json_encode( array(
                    'type' => 'error',
                    'msg'  => __( 'File type not recognized.', 'flotheme' )
                ) );
                die();
            }
        }
        ...
        private function getMissingFiles( $name, $fontname, $missing, $output, $subfolder ) {
            ...
            if ( count( $output ) == 1 && isset( $output['eot'] ) ) {
                echo json_encode( array(
                    'type' => 'error',
                    'msg'  => __( 'The font format .eot is not supported.', 'flotheme' )
                ) );
            ...
            if ( ! isset( $main ) ) {
                echo json_encode( array(
                    'type' => 'error',
                    'msg'  => __( 'No valid font file was found.', 'flotheme' )
                ) );
                $this->filesystem->delete( $temp, true, 'd' );
                $this->filesystem->delete( $this->upload_dir . $subfolder . $name . '/', true, 'd' );
                die();
            ...
            //         echo json_encode( array(
            //             'type' => 'error',
            //             'msg'  => __( 'You font could not be converted at this time. Please try again later.', 'flotheme' )
            //         ) );
            //         $this->filesystem->delete( $this->upload_dir . $subfolder . $name . '/', true, 'd' );
            //         $this->filesystem->delete( $temp, true, 'd' );
            //         die();
        ...
        function dynamic_section() {
            ...
            if ( ! isset( $this->parent->fontControl ) ) {
                $this->parent->sections[] = array(
                    'title'  => __( 'Custom Fonts', 'flotheme' ),
                    'icon'   => 'el-icon-font customfonts',
                    'id'     => 'redux_dynamic_font_control',
                    // Leave this as a blank section, no options just some intro text set above.
                    'fields' => array(
                        array(
                            'id' => 'divider_custom_fonts',
                            'title' => __('Custom Fonts', 'flotheme'),
                            'type' => 'info',
                            'class' => 'header_info',
                            'desc' => "Custom Fonts",
                            'doc_iframe' => "http://docs.flothemes.com/mimal-custom-fonts/"
                        ),
                    )
                );

                for ( $i = count( $this->parent->sections ); $i >= 1; $i -- ) {
                    if ( isset( $this->parent->sections[ $i ] ) && isset( $this->parent->sections[ $i ]['title'] ) && $this->parent->sections[ $i ]['title'] == __( 'Custom Fonts', 'flotheme' ) ) {
