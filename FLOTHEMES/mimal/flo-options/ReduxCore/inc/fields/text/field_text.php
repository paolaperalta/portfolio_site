...
if ( ! class_exists( 'ReduxFramework_text' ) ) {
    class ReduxFramework_text {
        ...
        function enqueue() {
            ...
            wp_enqueue_script('field_text_script',ReduxFramework::$_url . 'inc/fields/text/field_text_script.js', array('jquery'), '', true);
        }
        public function output() {
            $style = '';

            if ( ! empty( $this->value ) ) {
                $mode = ( isset( $this->field['mode'] ) && ! empty( $this->field['mode'] ) ? $this->field['mode'] : 'margin' );
