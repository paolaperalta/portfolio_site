...
//                        add_action( 'admin_enqueue_scripts', array( $this, '_enqueue_tracking' ) );
                    } else if ( ! isset( $this->options['tour'] ) && ( $this->parent->args['dev_mode'] == "true" || $this->parent->args['page_slug'] == "flo_options" ) ) {
            ...
            function tracking_request() {
                ...
                $content = '<h3>' . __( 'Help improve Our Panel', 'flotheme' ) . '</h3>';
                $content .= '<p>' . __( 'Please helps us improve our panel by allowing us to gather anonymous usage stats so we know which configurations, plugins and themes to test to ensure compatibility.', 'flotheme' ) . '</p>';
                $opt_arr = array(
                    'content'  => $content,
                    'position' => array( 'edge' => 'top', 'align' => 'center' )
                );
                $button2 = __( 'Allow tracking', 'flotheme' );

                $function2 = 'redux_store_answer("yes","' . $nonce . '")';
                $function1 = 'redux_store_answer("no","' . $nonce . '")';

                $this->print_scripts( $id, $opt_arr, __( 'Do not allow tracking', 'flotheme' ), $button2, $function2, $function1 );
            }
            ...
            function newsletter_request() {
                ...
                $content = '<h3>' . __( 'Welcome to the Redux Demo Panel', 'flotheme' ) . '</h3>';
                $content .= '<p><strong>' . __( 'Getting Started', 'flotheme' ) . '</strong><br>' . sprintf( __( 'This panel demonstrates the many features of Redux.  Before digging in, we suggest you get up to speed by reviewing %1$s.', 'flotheme' ), '<a href="' . 'http://' . 'docs.reduxframework.com/redux-framework/getting-started/" target="_blank">' . __( 'our documentation', 'flotheme' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Generator', 'flotheme' ) . '</strong><br>' . sprintf( __( 'Want to get a head start? Use the %1$s. It will create a customized boilerplate theme or a standalone admin folder complete with all things Redux (with the help of Underscores and TGM). Save yourself a headache and try it today.', 'flotheme' ), '<a href="' . 'http://' . 'generate.reduxframework.com/" target="_blank">' . __( 'Redux Generator', 'flotheme' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Extensions', 'flotheme' ) . '</strong><br>' . sprintf( __( 'Did you know we have extensions, which greatly enhance the features of Redux?  Visit our %1$s to learn more!', 'flotheme' ), '<a href="' . 'http://' . 'reduxframework.com/extensions/" target="_blank">' . __( 'extensions directory', 'flotheme' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Like Redux?', 'flotheme' ) . '</strong><br>' . sprintf( __( 'If so, please %1$s and consider making a %2$s to keep development of Redux moving forward.', 'flotheme' ), '<a target="_blank" href="' . 'http://' . 'wordpress.org/support/view/plugin-reviews/redux-framework">' . __( 'leave us a favorable review on WordPress.org', 'flotheme' ) . '</a>', '<a href="' . 'https://' . 'www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=N5AD7TSH8YA5U" target="_blank">' . __( 'donation', 'flotheme' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Newsletter', 'flotheme' ) . '</strong><br>' . __( 'If you\'d like to keep up to with all things Redux, please subscribe to our newsletter', 'flotheme' ) . ':</p>';
                $content .= '<form action="' . 'http://' . 'reduxframework.us7.list-manage2.com/subscribe/post?u=564f5178f6cc288064f332efd&amp;id=ace5bbc1f9&SOURCE=panel" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate><p style="text-align: center;"><label for="mce-EMAIL">' . __( 'Email address', 'flotheme' ) . ' </label><input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL"><input type="hidden" value="panel" name="SOURCE">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="' . __( 'Subscribe', 'flotheme' ) . '" name="subscribe" id="mc-embedded-subscribe" class="button button-primary"></p></form>';
                $opt_arr = array(
                    'content'      => $content,
                    'position'     => array( 'edge' => 'top', 'align' => 'center' ),
                    'pointerWidth' => 450
                ...
                $this->print_scripts( $id, $opt_arr, __( 'Close', 'flotheme' ), false, '', $function1 );
            }
