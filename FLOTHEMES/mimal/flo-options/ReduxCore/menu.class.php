<?php
    class menu extends Walker{
        ...
        function __construct( $args ) {
            ...
            $this -> nr_items_per_column = isset( $args['nr_items_per_column'] ) ? $args['nr_items_per_column'] : 9999;
        ...
        function start_lvl(&$output, $depth = 0, $args = array()) {
            ...
            $output .= "\n$indent" . $this ->beforesubm . "<ul class=\"header_main-nav_sub-menu " .$this -> subclass."\">\n";
        }
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
            ...
            if ($depth == 0){
                ...
                if($this -> limit == $this -> count){
                    $this -> need_more = true;
                    $output .= '<li class="header_main-nav_link menu-item ' . $this -> moreclass . ' '.$this->nr_columns.' no_description ">';
            ...
            if($this -> nr_items_per_column != 9999){
                // close and opend the UL to have menu devided in several columns
                if( $depth == 0 && ($this -> count % $this -> nr_items_per_column ) == 1 && $this -> count != 1){
                    $output .= '</ul><ul  '.$this -> classes .' >';    
                }    
            }
            ...
            if( $item -> menu_item_parent > 0 ){
                $class_names = ' class="header_main-nav_sub-menu_link menu-item ' . str_replace('menu-item ','', esc_attr( $class_names ) ) . '"';
            }else{
                $class_names = ' class="header_main-nav_link ' . esc_attr( $class_names ) . '"';
            }
            ...
            if(isset($flo_options) && isset($flo_options['flo-enable_menu_description']) && $flo_options['flo-enable_menu_description'] == 1 && !empty( $item->attr_title )){
                $item_output .= ' <span class="sub-title"> ';
        ...
        function get_terms_menu( $class = '' , $subclass = 'children' , $current = 'current' , $before = '' , $after = '' , $more = 'More' ){
            ...
            if( is_home() || is_front_page() ){
                $home = '<li class="'.$item.'-item '.$this -> current.' ' . $this -> firstitem . ' no_description">';
                $home .= '<a href="'.home_url().'">' . $this-> beforeitem . __('Home','flotheme') . $this->afteritem . '</a>' ;
                $home .= '</li>';
            }else{
                $home = '<li class="menu-item ' . $this -> firstitem . ' no_description">';
                $home .= '<a href="'.home_url().'">' . $this-> beforeitem . __('Home','flotheme') . $this->afteritem . '</a>' ;
                $home .= '</li>';
            }
            ...
            foreach( $terms as $key => $term ){
                ...
                if( $count == $this -> limit ){
                    ...
                    $menu .= '<ul class="header_main-nav_sub-menu '.$this -> subclass.' ' . $this -> moreclass . '">';
                }
                ...
                if( $count < $this -> limit ){
                    if( $is_term && !$this -> issetcurrent ){
                            $menu .= '<li class="header_main-nav_link '.$item.'-item '.$this -> current.' no_description" id="'.$item.'-item-' . $id . $this -> menu_item_id . '">';
                            $this -> issetcurrent = true;
                    }else{
                            $menu .= '<li class="header_main-nav_link '.$item.'-item '.$class.' no_description" id="'.$item.'-item-' . $id . $this -> menu_item_id . '">';
                    }
                }else{
                    $menu .= '<li class="header_main-nav_link '.$item.'-item '.$class.' no_description" id="'.$item.'-item-' . $id . $this -> menu_item_id . '">';
                }
        ...
        function get_childs( $args , $subclass ){
            ...
            if( count( $childs ) > 0 ) {
                ...
                $submenu  .= '<ul class="header_main-nav_sub-menu '.$subclass.'">';
                ...
                foreach( $childs as $key => $child ){
                    ...
                    $submenu .= '<li class="header_main-nav_link '.$item.'-item" id="'.$item.'-item-' . $id . '">';
