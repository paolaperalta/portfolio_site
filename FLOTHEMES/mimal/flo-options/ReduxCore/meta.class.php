            ...
            if( isset( $field_['type'] ) && $field_['type'] == 'attachment' && strlen( $field_['attach_type'] ) ){
                ...
                $attachment = array(
                    0 => array(
                        ...
                        'upload' => true,
                        'param' => 'readonly',
                        'hint' => __('To change attachment click on the icon' , 'flotheme'),
                        'lvisible' => false,
                        'evisible' => true
                    ),
                    1 => array(
                ...
                case 'text' :{
                    if( $fstyle == 'line' ){
                        if( isset( $post_link )  ){
                            if( $value > 0 ){
                                $p = get_post( $value );
                                $result .= '<p class="' . $lv_class . '">'  . $before . '<a href="post.php?post=' . $value . '&action=edit">' . __( 'Edit post' , 'flotheme' ) . ' - <strong>' . $p -> post_title . '</strong></a>' . $after . '</p>';
                                $result .= '<p class="' . $ev_class . '">'  . $before . '<a href="post.php?post=' . $value . '&action=edit">' . __( 'Edit post' , 'flotheme' ) . ' - <strong>' . $p -> post_title . '</strong></a>' . $after . '</p>';
                            }
                        }else{
                        ...
                        if( isset( $upload ) ){
                            $remove_translation = __( 'Remove' , 'flotheme' );
                            $result .= <<< endhtml
                                <a href="javascript:void(0);" onclick="jQuery( this ).parent().next().val('');jQuery( this ).prev().val('');">
                                    $remove_translation
                                </a>
endhtml;
                            $result .= '<input type="button" class="button-primary" value="'.__('Choose File','flotheme').'" onclick="javascript:act.upload_id(\'' . $res . '_' . $box . '\' , \''.$name.'\' , ' .$index . ' );"><br>';
                            wp_enqueue_media();
                        }
                        $result .= '</span>' . $fafter ;
                    }else{
                        if( isset( $upload ) ){
                            ...
                            $result .= '<label for="' . self::get_input_ID( $res , $box , $index , $name ) . '"  >'.$label.'</label><br />';
                            $result .= '<input type="text" id="' . self::get_input_ID( $res , $box , $index , $name ) . '" class="meta-box-field field-' . $box . ' ' . $fclasses . ' " name="'.$name.'" value="'.$value.'">';
                            $result .= '<input type="button" class="button-primary" value="'.__('Choose File','flotheme').'" onclick="javascript:act.upload_id(\'' . $res . '_' . $box . '\' , \''.$name.'\' , ' .$index . ' );">';
                    ...
                    default :{
                        if( is_array( $extra ) ){
                            ...
                            $result .= '<div class="generic-field generic-field-upload ' . $extra[2] . '">';
                                ...
                                $result .= '<input type="button" class="button-primary" value="'.__('Choose File','flotheme').'" onclick="javascript:act.upload(\'input#' . $name . '_path\' );">';
                            $result .= '</div>';
                        }
                    break;
                }
